#!/usr/bin/env python3
"""
Paper Survey 报告 Markdown → PDF 转换脚本 (WeasyPrint版)
用法: python md_to_pdf.py input.md output.pdf [--title "报告标题"] [--subtitle "副标题"] [--author "作者"]
...
def md_to_html(md_text, title="Paper Survey", subtitle="Paper Survey",
               meta_line="", author=""):
    ...
    if first_h1_match:
        ...
        if not title or title == "Paper Survey":
    ...
    # 构建封面
    author_html = f'<div class="meta">Author: {author}</div>' if author else ""
...
def main():
    parser = argparse.ArgumentParser(description="Paper Survey Markdown → PDF")
    parser.add_argument("input", help="输入的 Markdown 文件路径")
    parser.add_argument("output", help="输出的 PDF 文件路径")
    parser.add_argument("--title", default=None, help="报告标题")
    parser.add_argument("--subtitle", default="Paper Survey", help="副标题")
    ...
    for line in md_text.split("\n"):
        ...
        if any(kw in stripped for kw in ["研究领域", "时间范围", "论文数量", "Field", "Period", "Papers"]):
            meta_line = stripped
            break

    html = md_to_html(
        md_text,
        title=args.title or "Paper Survey",
